...
import { CLIP_SHOP_PLANS, getClipShopPlan } from "./plans";
...
const WORKSPACE_SLOTS = [1, 2, 3] as const;
const MAX_SAVED_SELECTIONS = 27;
...
const CLIPSHOP_QUESTIONS = [
  { question: "Preciso preencher os 9 espaços para começar?", answer: "Não. Você pode começar com apenas 1 Gancho, 1 Corpo e 1 CTA. Preencher os 9 espaços libera todas as 27 combinações." },
  ...
  { question: "Como as 27 combinações são formadas?", answer: "O Clip Shop combina 3 Ganchos × 3 Corpos × 3 CTAs, da sequência G1+C1+CTA1 até G3+C3+CTA3, sem duplicar trios." },
  { question: "Quantos vídeos posso selecionar por lote?", answer: "O Free permite 1 vídeo por lote, o Pro permite 5 e o Elite permite todas as 27 combinações de uma vez." },
...
function newProject(): ClipShopProject {
  ...
  return { id: crypto.randomUUID(), name: "Novo projeto", productName: "", status: "draft", strategy: "balanced", quality: "performance", audioPolicy: { ...DEFAULT_AUDIO_POLICY }, audioShuffle: createDefaultAudioShuffleProjectState(), compositionMode: "contain", exportNameTemplate: DEFAULT_EXPORT_NAME_TEMPLATE, clips: [], variations: [], createdAt: now, updatedAt: now };
}

function normalizeWorkspaceProject(project: ClipShopProject): ClipShopProject {
  const clips = project.clips.filter((clip) => clip.slot <= 3);
  ...
  return { ...project, audioPolicy: project.audioPolicy || { ...DEFAULT_AUDIO_POLICY }, audioShuffle, compositionMode: project.compositionMode || "contain", exportNameTemplate: project.exportNameTemplate || DEFAULT_EXPORT_NAME_TEMPLATE, clips, variations };
}
...
function ClipShopWorkspace({ currentUser, onExitToApp, onLogout, onUpgrade }: Props) {
  ...
  const recompute = (clips: ClipAsset[], strategy = project?.strategy || "balanced") => generateVariations(clips.filter((clip) => clip.slot <= 3), strategy);
  ...
  const addFiles = async (category: ClipCategory, files: File[], forcedSlot?: number) => {
    ...
    if (files.length > accepted.length) setError(`${files.length - accepted.length} arquivo(s) excedente(s) não foram adicionados porque esta coluna possui apenas 3 slots.`);
  ...
  const workspaceClips = project?.clips.filter((clip) => clip.slot <= 3 && clip.status !== "error") ?? [];
    ...
    try {
      ...
      for (let index = 0; index < targets.length; index += 1) {
        ...
        try {
          ...
          const rendered = await renderVariation(variation, currentProject?.clips ?? [], currentProject ? { audioPolicy: currentProject.audioPolicy, compositionMode: currentProject.compositionMode, quality: currentProject.quality } : undefined, (progress) => updateProject((current) => ({ ...current, variations: current.variations.map((item) => item.id === variation.id ? { ...item, progress: Math.round(progress.progress * 75) } : item) })), controller.signal);
  ...
  const runTest = async () => {
    ...
    try {
      const rendered = await renderVariation(selected[0], project.clips, { audioPolicy: project.audioPolicy, compositionMode: project.compositionMode, quality: project.quality });
  ...
  if (!project) return <Shell currentUser={currentUser} onExit={onExitToApp} onLogout={onLogout} usage={usage}>
    <div className="mx-auto max-w-6xl py-8">
      <div className="mb-8"><p className="text-xs font-black uppercase tracking-[.28em] text-amber-500">Criativos em escala</p><h1 className="mt-2 text-3xl font-black text-white sm:text-5xl">Um produto. <span className="text-amber-400">27 maneiras de vender.</span></h1><p className="mt-4 max-w-2xl text-sm text-zinc-400">Combine Ganchos, Corpos e CTAs. Seus vídeos permanecem no dispositivo durante o processamento local.</p></div>
  ...
  return <Shell currentUser={currentUser} onExit={onExitToApp} onLogout={onLogout} usage={usage}>
    <div className="mx-auto max-w-[1320px] py-7 sm:py-10">
      ...
      <section className="relative mb-8 overflow-hidden rounded-[2rem] border border-amber-500/15 bg-gradient-to-br from-zinc-900 via-zinc-950 to-black p-6 sm:p-9">
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-8"><div className="min-w-0 flex-1"><p className="text-[10px] font-black uppercase tracking-[.3em] text-amber-500">Criativos em escala</p><input value={project.name} onChange={(event) => updateProject({ name: event.target.value })} className="mt-3 w-full max-w-2xl bg-transparent text-3xl font-black tracking-tight text-white outline-none sm:text-5xl" /><p className="mt-2 text-xl font-black text-amber-400 sm:text-3xl">27 maneiras de vender.</p><input value={project.productName} onChange={(event) => updateProject({ productName: event.target.value })} placeholder="Nome do produto (opcional)" className="mt-5 block w-full max-w-xl border-b border-zinc-800 bg-transparent pb-2 text-xs text-zinc-400 outline-none transition focus:border-amber-500/60" /></div><div className="min-w-44"><div className="flex items-end gap-1"><strong className="text-4xl font-black text-white">{workspaceClips.length}</strong><span className="mb-1 text-sm font-bold text-zinc-600">/9</span></div><p className="mt-1 text-[9px] font-bold uppercase tracking-[.18em] text-zinc-500">trechos enviados</p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-zinc-800"><div className="h-full rounded-full bg-gradient-to-r from-amber-600 to-amber-300 transition-all" style={{ width: `${workspaceClips.length / 9 * 100}%` }} /></div></div></div>
      </section>
      ...
      <section id="clips" className="mb-8"><div className="mb-5 flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.3em] text-amber-500">01 · Monte seu criativo</p><h2 className="mt-2 text-2xl font-black text-white">Envie os nove trechos</h2><p className="mt-1 text-xs text-zinc-500">Arraste arquivos para uma coluna ou use o upload geral para classificá-los.</p></div><label className="cursor-pointer rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-xs font-black text-amber-300"><input type="file" multiple accept="video/mp4,video/quicktime,.mp4,.mov" className="hidden" onChange={(event) => { const files = Array.from(event.target.files ?? []); setStagedFiles(files.map((file, index) => ({ id: `${file.name}-${file.lastModified}-${index}`, file, category: index < 3 ? "hook" : index < 6 ? "body" : "cta" }))); event.target.value = ""; }} />Upload geral e classificar</label></div><div className="grid gap-4 lg:grid-cols-3">{(["hook", "body", "cta"] as const).map((category) => <ClipColumnWithAudio key={category} category={category} clips={project.clips} busySlots={busySlots} onFiles={(files) => addFiles(category, files)} onSlotFile={(slot, file) => addFiles(category, [file], slot)} onRemove={removeClip} onRename={(clip, semanticName) => updateProject((current) => ({ ...current, clips: current.clips.map((item) => item.id === clip.id ? { ...item, semanticName } : item) }))} onMove={moveClip} onMute={(clip) => updateProject((current) => ({ ...current, clips: current.clips.map((item) => item.id === clip.id ? { ...item, muted: !item.muted } : item) }))} />)}</div></section>
      <ProjectAudioShufflePanel project={project} onChange={updateProject} />
      <section id="combinations" className="mb-8 rounded-[2rem] border border-amber-500/10 bg-[#0c0c0c] p-5 shadow-2xl shadow-black/30 sm:p-8"><div className="mb-6 flex flex-wrap items-end justify-between gap-5"><div><p className="text-[10px] font-black uppercase tracking-[.3em] text-amber-500">03 · Combinações</p><h2 className="mt-2 text-2xl font-black text-white sm:text-3xl">Suas combinações</h2><p className="mt-2 text-xs text-zinc-500">Preencha os 9 espaços para liberar as 27 combinações. Seu plano permite {maxSelectionPerGeneration} por lote.</p></div><div className="flex flex-wrap items-center gap-3"><div className="flex items-center gap-3 text-[9px] font-bold"><span className="text-amber-300">● Gancho</span><span className="text-amber-400">● Corpo</span><span className="text-amber-500">● CTA</span></div><button onClick={() => updateProject((current) => ({ ...current, variations: current.variations.map((item, index) => ({ ...item, selected: index < Math.min(maxSelectionPerGeneration, current.variations.length) })) }))} className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-3 py-2 text-[10px] font-black text-amber-400 transition hover:bg-amber-500/10">Selecionar {Math.min(maxSelectionPerGeneration, project.variations.length)} ({maxSelectionPerGeneration} por lote)</button><button onClick={() => updateProject((current) => ({ ...current, variations: current.variations.map((item) => ({ ...item, selected: false })) }))} className="text-[10px] font-bold text-zinc-500 hover:text-white">Limpar</button></div></div><div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">{variationDesk.map((row) => <VariationCard key={row.number} number={row.number} hookSlot={row.hookSlot} bodySlot={row.bodySlot} ctaSlot={row.ctaSlot} variation={row.variation} selectionFull={selected.length >= maxSelectionPerGeneration} onToggle={() => row.variation && toggleVariation(row.variation.id)} onPreview={() => row.variation && setPreview(row.variation)} />)}</div>
      ...
      <section className="mb-6 rounded-3xl border border-zinc-800 bg-zinc-950/50 p-5 sm:p-6"><div className="mb-1"><p className="text-[10px] font-black uppercase tracking-[.25em] text-amber-500">Resultados</p><h2 className="mt-1 text-xl font-black text-white">Seus vídeos prontos</h2></div>
        {outputs.length > 0 && <div className="mt-6"><div className="mb-3 flex items-center justify-between"><h3 className="font-bold text-white">Seus criativos</h3><button onClick={() => downloadText(buildManifest(project.name, project.variations.filter((item) => outputs.some((output) => output.variationId === item.id)), project.clips), `${sanitizeFilePart(project.name)}-manifesto.csv`, "text/csv;charset=utf-8")} className="flex items-center gap-1.5 text-xs font-bold text-amber-400"><Download className="h-4 w-4" />Manifesto CSV</button></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{outputs.map((output) => <div key={output.id} className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900"><video src={output.objectUrl} controls preload="metadata" className="aspect-[9/16] max-h-72 w-full bg-black object-contain" /><div className="p-4"><p className="truncate text-xs font-bold text-white">{output.fileName}</p><p className="mt-1 text-[10px] text-zinc-500">{formatBytes(output.size)} · {output.duration.toFixed(1)}s</p><div className="mt-3 flex gap-2"><a href={output.objectUrl} download={output.fileName} className="flex flex-1 items-center justify-center gap-1.5 rounded-lg bg-amber-400 py-2 text-[10px] font-black text-zinc-950"><Download className="h-3.5 w-3.5" />MP4</a>{output.audioObjectUrl && <a href={output.audioObjectUrl} download={`${output.fileName.replace(/\\.mp4$/i, "")}-audio.wav`} className="flex items-center justify-center gap-1.5 rounded-lg border border-amber-500/30 px-3 py-2 text-[10px] font-black text-amber-300"><Volume2 className="h-3.5 w-3.5" />WAV</a>}<button onClick={() => setDeleteTarget({ kind: "output", output })} title={`Excluir ${output.fileName}`} className="rounded-lg border border-zinc-700 px-2.5 text-zinc-500 transition hover:border-red-500/30 hover:bg-red-500/5 hover:text-red-400"><Trash2 className="h-3.5 w-3.5" /></button></div></div></div>)}</div></div>}
...
function OnboardingDialog({ step, onStep, onClose }: { step: number; onStep: (step: number) => void; onClose: () => void }) {
  const steps = [
    { number: "01", title: "Envie seus trechos", text: "Adicione até 3 Ganchos, 3 Corpos e 3 CTAs. Você pode começar apenas com um clipe de cada categoria.", icon: <Upload className="h-6 w-6" /> },
    { number: "02", title: "Escolha as combinações", text: "A mesa organiza até 27 combinações. Selecione o lote permitido pelo seu plano e use o botão de play para revisar a sequência.", icon: <Layers3 className="h-6 w-6" /> },
...
function ProjectMediaSettings({ project, onChange }: { project: ClipShopProject; onChange: (change: Partial<ClipShopProject>) => void }) {
  ...
  return <section className="mb-8 grid gap-5 rounded-3xl border border-zinc-800 bg-zinc-950/60 p-5 lg:grid-cols-[1fr_220px] sm:p-6"><div><p className="text-[10px] font-black uppercase tracking-[.25em] text-amber-500">Mídia do projeto</p><h2 className="mt-2 text-xl font-black text-white">Enquadramento e áudio</h2><div className="mt-5"><p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-zinc-500">Vídeos fora de 9:16</p><div className="flex flex-wrap gap-2">{modes.map((mode) => <button key={mode.value} onClick={() => onChange({ compositionMode: mode.value })} className={`rounded-xl border px-3 py-2 text-[10px] font-bold ${project.compositionMode === mode.value ? "border-amber-400/50 bg-amber-400/10 text-amber-300" : "border-zinc-800 text-zinc-500"}`}>{mode.label}</button>)}</div></div><div className="mt-5 grid gap-3 sm:grid-cols-4"><label className="text-[10px] font-bold text-zinc-500">Política<select value={project.audioPolicy.mode} onChange={(event) => onChange({ audioPolicy: { ...project.audioPolicy, mode: event.target.value as ClipShopProject["audioPolicy"]["mode"] } })} className="mt-1.5 w-full rounded-xl border border-zinc-800 bg-zinc-900 px-3 py-2 text-xs text-white"><option value="preserve">Preservar</option><option value="normalize">Normalizar</option><option value="mute">Sem áudio</option></select></label><NumberSetting label="Loudness alvo" value={project.audioPolicy.targetLoudnessDb} suffix="dB" min={-24} max={-8} onChange={(value) => onChange({ audioPolicy: { ...project.audioPolicy, targetLoudnessDb: value } })} /><NumberSetting label="Teto de pico" value={project.audioPolicy.peakDb} suffix="dB" min={-6} max={0} onChange={(value) => onChange({ audioPolicy: { ...project.audioPolicy, peakDb: value } })} /><NumberSetting label="Fade nas emendas" value={project.audioPolicy.fadeMs} suffix="ms" min={0} max={100} onChange={(value) => onChange({ audioPolicy: { ...project.audioPolicy, fadeMs: value } })} /></div><p className="mt-3 text-[9px] leading-4 text-zinc-600">Normalizar cria AAC estéreo 48 kHz, adiciona silêncio onde faltar áudio, aproxima o nível ao alvo e limita picos para evitar clipping.</p></div><div className="relative mx-auto aspect-[9/16] w-36 overflow-hidden rounded-2xl border border-zinc-800 bg-black lg:w-full">{preview ? <><img src={preview} alt="Prévia do enquadramento" className={`h-full w-full ${project.compositionMode === "cover" ? "object-cover" : "object-contain"}`} />{project.compositionMode === "blur" && <><img src={preview} alt="" className="absolute inset-0 h-full w-full scale-110 object-cover blur-xl opacity-60" /><img src={preview} alt="" className="absolute inset-0 h-full w-full object-contain" /></>}</> : <div className="grid h-full place-items-center px-4 text-center text-[9px] text-zinc-700">Adicione um clipe para visualizar</div>}<span className="absolute bottom-2 left-2 rounded bg-black/70 px-2 py-1 text-[8px] font-bold text-white">Prévia</span></div></section>;
}
...
function PlanModal({ currentPlan, usage, onClose, onUpgrade }: { currentPlan: ClipShopPlanKey; usage: ClipShopUsage | null; onClose: () => void; onUpgrade: () => void }) {
  ...
  return <div className="fixed inset-0 z-[120] grid place-items-center overflow-y-auto bg-black/85 p-4 backdrop-blur-md" onMouseDown={onClose}><div role="dialog" aria-modal="true" aria-labelledby="clipshop-plan-title" className="my-5 w-full max-w-4xl rounded-[2rem] border border-amber-500/20 bg-zinc-950 p-5 shadow-[0_35px_120px_rgba(0,0,0,.8)] sm:p-8" onMouseDown={(event) => event.stopPropagation()}><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[.3em] text-amber-500">Plano e consumo</p><h2 id="clipshop-plan-title" className="mt-2 text-2xl font-black text-white sm:text-3xl">Seu Clip Shop é {plan.name}</h2><p className="mt-2 text-sm text-zinc-500">{usage?.used ?? 0} de {plan.monthlyVideos} vídeos utilizados neste período.</p></div><button onClick={onClose} aria-label="Fechar planos" className="rounded-xl border border-zinc-800 p-2 text-zinc-500 hover:text-white"><X className="h-5 w-5" /></button></div><div className="mt-7 grid gap-4 md:grid-cols-3">{Object.values(CLIP_SHOP_PLANS).map((item) => { const active = item.key === currentPlan; return <article key={item.key} className={`relative rounded-2xl border p-5 ${active ? "border-amber-400 bg-amber-500/[.07]" : "border-zinc-800 bg-zinc-900/40"}`}>{active && <span className="absolute right-4 top-4 rounded-md bg-amber-400 px-2 py-1 text-[8px] font-black uppercase text-black">Plano atual</span>}<h3 className="text-lg font-black text-white">{item.name}</h3><p className="mt-2 min-h-10 text-xs leading-5 text-zinc-500">{item.description}</p><strong className="mt-5 block text-3xl font-black text-amber-400">{item.monthlyVideos}</strong><span className="text-[10px] uppercase tracking-wider text-zinc-600">vídeos por mês</span><ul className="mt-5 space-y-2 text-xs text-zinc-300"><li className="flex gap-2"><Check className="h-4 w-4 text-amber-400" />Até {item.combinations} combinações</li><li className="flex gap-2"><Check className="h-4 w-4 text-amber-400" />Até {item.batchSize} por lote</li><li className="flex gap-2"><Check className="h-4 w-4 text-amber-400" />Navegação completa</li><li className="flex gap-2"><Check className="h-4 w-4 text-amber-400" />Prévia sem consumo</li></ul>{!active && item.key !== "free" && <button onClick={onUpgrade} className="mt-5 w-full rounded-xl bg-amber-400 py-3 text-xs font-black text-zinc-950 hover:bg-amber-300">Fazer upgrade</button>}</article>; })}</div><div className="mt-6 rounded-2xl border border-zinc-800 bg-black/30 p-4 text-xs leading-6 text-zinc-500"><strong className="text-zinc-300">Como funciona:</strong> somente vídeos concluídos consomem o limite. Validação, prévia e processamento com erro não consomem. O saldo é renovado mensalmente e excluir um resultado não devolve o uso.</div></div></div>;
}
...
function Shell({ currentUser, onExit, onLogout, usage, children }: { currentUser: Props["currentUser"]; onExit: () => void; onLogout: () => void; usage?: ClipShopUsage | null; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#070707] text-zinc-200"><header className="sticky top-0 z-40 border-b border-zinc-900 bg-[#070707]/90 px-4 backdrop-blur-xl"><div className="mx-auto flex h-16 max-w-[1500px] items-center gap-3"><Logo size={31} /><div className="font-black text-white">UMBRA <span className="text-amber-400">CLIP SHOP</span></div><div className="ml-auto flex items-center gap-2"><span className="hidden rounded-lg border border-zinc-800 px-2.5 py-1.5 text-[10px] font-bold uppercase text-zinc-400 sm:block">{currentUser.plan || "Free"}</span>{usage && usage.limit >= 0 && <span className="rounded-lg bg-amber-500/10 px-2.5 py-1.5 text-[10px] font-bold text-amber-400">{usage.remaining}/{usage.limit} vídeos</span>}<a href="#/dashboard" onClick={(event) => { event.preventDefault(); openDashboard(); }} title="Voltar ao Umbra Copy" className="flex items-center gap-2 rounded-lg border border-zinc-800 px-3 py-2 text-[10px] font-black text-zinc-300 transition hover:border-amber-500/30 hover:text-amber-400"><ArrowLeft className="h-4 w-4" /><span className="hidden sm:inline">Umbra Copy</span></a><button onClick={onLogout} title="Sair da conta" className="rounded-lg border border-zinc-800 p-2 text-zinc-400 hover:text-red-400"><LogOut className="h-4 w-4" /></button></div></div></header><main className="px-4 sm:px-6">{children}</main></div>;
}
...
function ReviewModal({ project, compatibility, selected, totalDuration, estimatedBytes, usage, testOutputUrl, testBusy, onTest, onClose, onConfirm, onUpgrade }: { project: ClipShopProject; compatibility: CompatibilityReport | null; selected: Variation[]; totalDuration: number; estimatedBytes: number; usage: ClipShopUsage | null; testOutputUrl: string; testBusy: boolean; onTest: () => void; onClose: () => void; onConfirm: () => void; onUpgrade: () => void }) {
  ...
  const balanceAfter = usage ? usage.remaining - selected.length : null;
  const blocked = Boolean((usage && usage.remaining < selected.length) || !filesAccessible || !storageOk || compatibility?.supported === false);
...
function buildVariationDesk(clips: ClipAsset[], variations: Variation[]) {
  const byCategoryAndSlot = new Map(clips.filter((clip) => clip.slot <= 3 && clip.status !== "error").map((clip) => [`${clip.category}-${clip.slot}`, clip]));
  const byTrio = new Map(variations.map((variation) => [`${variation.hookId}|${variation.bodyId}|${variation.ctaId}`, variation]));
  const rows: Array<{ number: number; hookSlot: number; bodySlot: number; ctaSlot: number; variation?: Variation }> = [];
  let number = 1;
  for (const hookSlot of WORKSPACE_SLOTS) for (const bodySlot of WORKSPACE_SLOTS) for (const ctaSlot of WORKSPACE_SLOTS) {
    const hook = byCategoryAndSlot.get(`hook-${hookSlot}`);
    const body = byCategoryAndSlot.get(`body-${bodySlot}`);
    const cta = byCategoryAndSlot.get(`cta-${ctaSlot}`);
    const variation = hook && body && cta ? byTrio.get(`${hook.id}|${body.id}|${cta.id}`) : undefined;
    rows.push({ number, hookSlot, bodySlot, ctaSlot, variation });
    number += 1;
  }
  return rows;
}
