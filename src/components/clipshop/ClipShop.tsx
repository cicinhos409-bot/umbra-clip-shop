...
import { getVariationDifferentiationLevel, getVariationDiversityLevel } from "./services/variation-recipe";
...
function newProject(): ClipShopProject {
  ...
  return { id: crypto.randomUUID(), name: "Novo projeto", productName: "", status: "draft", strategy: "balanced", quality: "performance", audioPolicy: { ...DEFAULT_AUDIO_POLICY }, audioShuffle: createDefaultAudioShuffleProjectState(), compositionMode: "contain", visualVariationsEnabled: true, advancedDiversityEnabled: true, mp4MetadataEnabled: true, outputAspectRatio: "9:16", headlineText: "", captionText: "", exportNameTemplate: DEFAULT_EXPORT_NAME_TEMPLATE, clips: [], variations: [], createdAt: now, updatedAt: now };
}
...
function normalizeWorkspaceProject(project: ClipShopProject): ClipShopProject {
  ...
  const variations = generateVariations(clips, project.strategy).map((variation) => {
    ...
    return saved ? { ...variation, ...saved, number: variation.number, recipe: variation.recipe, selected, ...(saved.status === "processing" ? { status: "queued" as const, progress: 0, error: "Geração interrompida; pronta para continuar." } : {}) } : variation;
  });
...
function ClipShopWorkspace({ currentUser, onExitToApp, onLogout, onUpgrade }: Props) {
  ...
  const stageUploadFiles = (files: File[]) => {
    const accepted = files.filter((file) => file.type === "video/mp4" || file.type === "video/quicktime" || /\.(mp4|mov)$/i.test(file.name));
    if (!accepted.length) {
      setError("Selecione vídeos MP4 ou MOV pela galeria ou arraste os arquivos para esta área.");
      return;
    }
    setError(accepted.length < files.length ? "Alguns arquivos foram ignorados porque não são vídeos MP4 ou MOV." : "");
    setStagedFiles(accepted.map((file, index) => ({
      id: `${file.name}-${file.lastModified}-${index}`,
      file,
      category: index < MAX_WORKSPACE_SLOTS ? "hook" : index < MAX_WORKSPACE_SLOTS * 2 ? "body" : "cta",
    })));
  };
  ...
  const createChampionCombinations = (clip: ClipAsset) => {
    if (!project) return;
    const matching = project.variations.filter((variation) =>
      clip.category === "hook" ? variation.hookId === clip.id
        : clip.category === "body" ? variation.bodyId === clip.id
          : variation.ctaId === clip.id,
    );
    if (!matching.length) {
      setError("Adicione pelo menos 1 Gancho, 1 Corpo e 1 CTA para criar combinações com este trecho.");
      return;
    }
    const chosenIds = new Set(matching.slice(0, maxSelectionPerGeneration).map((variation) => variation.id));
    setCombinationFilters((current) => ({ ...current, [clip.category]: clip.slot }));
    setGenerationQuantity(chosenIds.size);
    updateProject((current) => ({ ...current, variations: current.variations.map((variation) => ({ ...variation, selected: chosenIds.has(variation.id) })) }));
    setError("");
    window.requestAnimationFrame(() => document.getElementById("combinations")?.scrollIntoView({ behavior: "smooth" }));
  };
  ...
  const runQueue = async (recovery?: QueueSnapshot, explicitIds?: string[]) => {
    ...
    const unorderedTargets = project.variations.filter((item) => targetIds.includes(item.id));
    const targets = project.advancedDiversityEnabled ? orderVariationsForDiversity(unorderedTargets) : unorderedTargets;
    ...
    try {
      ...
      for (let index = 0; index < targets.length; index += 1) {
        ...
        try {
          ...
          const rendered = await renderVariation(variation, currentProject?.clips ?? [], currentProject ? { audioPolicy: currentProject.audioPolicy, compositionMode: currentProject.compositionMode, quality: currentProject.quality, visualVariationsEnabled: currentProject.visualVariationsEnabled && currentProject.advancedDiversityEnabled, mp4MetadataEnabled: currentProject.mp4MetadataEnabled, outputAspectRatio: currentProject.outputAspectRatio, headlineText: currentProject.headlineText, captionText: currentProject.captionText } : undefined, (progress) => updateProject((current) => ({ ...current, variations: current.variations.map((item) => item.id === variation.id ? { ...item, progress: Math.round(progress.progress * 75) } : item) })), controller.signal);
  ...
  const runTest = async () => {
    ...
    try {
      const rendered = await renderVariation(selected[0], project.clips, { audioPolicy: project.audioPolicy, compositionMode: project.compositionMode, quality: project.quality, visualVariationsEnabled: project.visualVariationsEnabled && project.advancedDiversityEnabled, mp4MetadataEnabled: project.mp4MetadataEnabled, outputAspectRatio: project.outputAspectRatio, headlineText: project.headlineText, captionText: project.captionText });
  ...
  return <Shell currentUser={currentUser} onExit={onExitToApp} onLogout={onLogout} usage={usage}>
    <div className="mx-auto max-w-[1320px] py-7 sm:py-10">
      ...
      <section id="clips" className="mb-8"><div className="mb-5"><div><p className="text-[10px] font-black uppercase tracking-[.3em] text-amber-500">01 · Monte seu criativo</p><h2 className="mt-2 text-2xl font-black text-white">Envie até {MAX_WORKSPACE_CLIPS} trechos</h2><p className="mt-1 text-xs text-zinc-500">Até {MAX_WORKSPACE_SLOTS} Ganchos, {MAX_WORKSPACE_SLOTS} Corpos e {MAX_WORKSPACE_SLOTS} CTAs.</p></div><div onDragEnter={(event) => { event.preventDefault(); setUploadDragActive(true); }} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "copy"; setUploadDragActive(true); }} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setUploadDragActive(false); }} onDrop={(event) => { event.preventDefault(); setUploadDragActive(false); stageUploadFiles(Array.from(event.dataTransfer.files)); }} className={`mt-5 flex flex-col items-center justify-center rounded-2xl border-2 border-dashed px-5 py-7 text-center transition ${uploadDragActive ? "border-amber-400 bg-amber-500/10" : "border-zinc-700 bg-zinc-950/60 hover:border-amber-500/40"}`}><Upload className={`h-7 w-7 ${uploadDragActive ? "text-amber-300" : "text-zinc-600"}`} /><p className="mt-3 text-sm font-black text-white">{uploadDragActive ? "Solte os vídeos aqui" : "Envie vídeos pelo celular ou computador"}</p><p className="mt-1 text-[10px] text-zinc-500">Escolha na galeria do celular ou arraste vários arquivos MP4/MOV para esta área.</p><label className="mt-4 cursor-pointer rounded-xl bg-amber-400 px-5 py-3 text-[10px] font-black text-zinc-950 hover:bg-amber-300"><input type="file" multiple accept="video/mp4,video/quicktime,.mp4,.mov" className="hidden" onChange={(event) => { stageUploadFiles(Array.from(event.target.files ?? [])); event.target.value = ""; }} />Escolher vídeos na galeria</label></div></div><div className="grid gap-4 lg:grid-cols-3">{(["hook", "body", "cta"] as const).map((category) => <ClipColumnWithAudio key={category} category={category} clips={project.clips} busySlots={busySlots} onFiles={(files) => addFiles(category, files)} onSlotFile={(slot, file) => addFiles(category, [file], slot)} onRemove={removeClip} onRename={(clip, semanticName) => updateProject((current) => ({ ...current, clips: current.clips.map((item) => item.id === clip.id ? { ...item, semanticName } : item) }))} onMove={moveClip} onMute={(clip) => updateProject((current) => ({ ...current, clips: current.clips.map((item) => item.id === clip.id ? { ...item, muted: !item.muted } : item) }))} onChampion={createChampionCombinations} />)}</div></section>
      ...
      <section id="combinations" className="mb-8 rounded-[2rem] border border-amber-500/10 bg-[#0c0c0c] p-5 shadow-2xl shadow-black/30 sm:p-8">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2 text-[9px]"><span className="font-black uppercase tracking-wider text-zinc-600">Diferenciação:</span><span className="rounded-md bg-emerald-500/10 px-2 py-1 font-black text-emerald-400">Alta · nenhum elemento igual</span><span className="rounded-md bg-amber-500/10 px-2 py-1 font-black text-amber-400">Média · reutiliza elementos</span><span className="rounded-md bg-red-500/10 px-2 py-1 font-black text-red-400">Baixa · trio e visual repetidos</span></div>
        <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">{filteredVariationDesk.map((row) => <VariationCard key={row.number} number={row.number} hookSlot={row.hookSlot} bodySlot={row.bodySlot} ctaSlot={row.ctaSlot} variation={row.variation} selectedVariations={selected} selectionFull={selected.length >= maxSelectionPerGeneration} onToggle={() => row.variation && toggleVariation(row.variation.id)} onPreview={() => row.variation && setPreview(row.variation)} />)}</div>
  ...
  return <section className="mb-8 grid gap-5 rounded-3xl border border-zinc-800 bg-zinc-950/60 p-5 lg:grid-cols-[1fr_220px] sm:p-6">
    <div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        <ToggleSetting label="Diversidade avançada" description="Evita duplicatas, alterna enquadramento/textos e distancia vídeos semelhantes na fila." checked={project.advancedDiversityEnabled} onChange={(checked) => onChange({ advancedDiversityEnabled: checked })} />
...
function PrivacyRetentionNotice() {
  return <section className="mt-6 rounded-3xl border border-emerald-500/15 bg-emerald-500/[.035] p-5 sm:p-6"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-emerald-500/10 text-emerald-400"><Check className="h-5 w-5" /></span><div><p className="text-[10px] font-black uppercase tracking-[.22em] text-emerald-400">Privacidade e retenção</p><h2 className="mt-1 text-lg font-black text-white">Registros técnicos são apagados após 3 dias</h2><p className="mt-2 max-w-3xl text-xs leading-5 text-zinc-500">Informações individuais de processamento — como status, plano usado, quantidade e horários da geração — são removidas automaticamente depois de 3 dias. Durante o ciclo atual, permanece apenas um total mensal compacto para aplicar corretamente o limite do seu plano.</p><p className="mt-2 text-xs leading-5 text-zinc-500">Seus vídeos e projetos continuam armazenados somente neste dispositivo e não são apagados por essa limpeza. Eles permanecem disponíveis até você excluir um resultado, projeto ou limpar o armazenamento local.</p></div></div></section>;
}
...
function ClipColumnWithAudio(props: React.ComponentProps<typeof ClipColumn> & { onMute: (clip: ClipAsset) => void; onChampion: (clip: ClipAsset) => void }) {
  const categoryClips = props.clips.filter((clip) => clip.category === props.category).sort((a, b) => a.slot - b.slot);
  const { onMute, onChampion, ...columnProps } = props;
  return <div><ClipColumn {...columnProps} /><div className="mt-2 grid grid-cols-3 gap-2">{WORKSPACE_SLOTS.map((slot) => { const clip = categoryClips.find((item) => item.slot === slot); return <button key={slot} disabled={!clip} onClick={() => clip && onMute(clip)} className={`flex items-center justify-center gap-1 rounded-lg border py-2 text-[8px] font-bold ${clip?.muted ? "border-red-500/30 bg-red-500/10 text-red-300" : "border-zinc-800 text-zinc-600"} disabled:opacity-30`}>{clip?.muted ? <VolumeX className="h-3 w-3" /> : <Volume2 className="h-3 w-3" />}{clip?.muted ? `Slot ${slot} mudo` : `Áudio ${slot}`}</button>; })}</div>{categoryClips.map((clip) => <button type="button" key={clip.id} onClick={() => onChampion(clip)} className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl border border-amber-500/25 bg-amber-500/[.06] px-3 py-2.5 text-[9px] font-black text-amber-300 transition hover:border-amber-400/50 hover:bg-amber-500/10"><Sparkles className="h-3.5 w-3.5" />Criar novas combinações com {clip.semanticName || `${categoryInfo[clip.category].singular} ${clip.slot}`} campeão</button>)}</div>;
}
...
function VariationCard({ number, hookSlot, bodySlot, ctaSlot, variation, selectedVariations, selectionFull, onToggle, onPreview }: { number: number; hookSlot: number; bodySlot: number; ctaSlot: number; variation?: Variation; selectedVariations: Variation[]; selectionFull: boolean; onToggle: () => void; onPreview: () => void }) {
  const enabled = Boolean(variation);
  const selected = variation?.selected ?? false;
  const differentiation = variation ? getVariationDifferentiationLevel(variation, selectedVariations) : "high";
  const differentiationInfo = differentiation === "high" ? { label: "Alta", title: "Bem diferente das combinações selecionadas", style: "bg-emerald-500/10 text-emerald-400" } : differentiation === "medium" ? { label: "Média", title: "Reutiliza um ou dois elementos de outra combinação", style: "bg-amber-500/10 text-amber-400" } : { label: "Baixa", title: "Repete Gancho, Corpo, CTA e tratamento visual", style: "bg-red-500/10 text-red-400" };
  return <div className={`rounded-xl border p-3 transition ${selected ? "border-amber-400/50 bg-amber-400/[.07] shadow-lg shadow-amber-950/10" : enabled ? "border-zinc-800 bg-zinc-900/50 hover:border-zinc-700" : "border-zinc-900 bg-black/20 opacity-40"}`}><div className="flex items-center gap-2"><button disabled={!enabled || (selectionFull && !selected)} onClick={onToggle} aria-label={`Selecionar variação ${number}`} className={`grid h-5 w-5 shrink-0 place-items-center rounded-md border transition ${selected ? "border-amber-400 bg-amber-400 text-black" : "border-zinc-700 bg-zinc-950"} disabled:cursor-not-allowed`}>{selected && <Check className="h-3 w-3" />}</button><span className="w-7 text-[9px] font-black text-zinc-600">{String(number).padStart(3, "0")}</span><div className="flex min-w-0 flex-1 items-center gap-1.5"><span className="rounded-md bg-amber-300/10 px-2 py-1 text-[9px] font-black text-amber-300">G{hookSlot}</span><span className="rounded-md bg-amber-400/10 px-2 py-1 text-[9px] font-black text-amber-400">C{bodySlot}</span><span className="rounded-md bg-amber-500/10 px-2 py-1 text-[9px] font-black text-amber-500">CTA{ctaSlot}</span></div><span title={differentiationInfo.title} className={`rounded-md px-2 py-1 text-[8px] font-black ${differentiationInfo.style}`}>{differentiationInfo.label}</span><button disabled={!enabled} onClick={onPreview} aria-label={`Visualizar variação ${number}`} className="rounded-lg p-1.5 text-zinc-600 transition hover:bg-amber-500/10 hover:text-amber-400 disabled:cursor-not-allowed disabled:opacity-30"><Play className="h-3 w-3 fill-current" /></button></div>{variation && variation.status !== "idle" && <div className="mt-2"><div className="h-1 overflow-hidden rounded bg-zinc-800"><div className={`h-full ${variation.status === "error" ? "bg-red-500" : "bg-amber-400"}`} style={{ width: `${variation.progress}%` }} /></div><p className={`mt-1 truncate text-[8px] ${variation.status === "error" ? "text-red-400" : "text-zinc-600"}`}>{variation.error || variation.status}</p></div>}</div>;
}
