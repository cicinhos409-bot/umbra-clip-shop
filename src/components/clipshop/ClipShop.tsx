  ...
  return <Shell currentUser={currentUser} onExit={onExitToApp} onLogout={onLogout} usage={usage}>
    <div className="mx-auto max-w-[1320px] py-7 sm:py-10">
      ...
      <section id="combinations" className="mb-8 rounded-[2rem] border border-amber-500/10 bg-[#0c0c0c] p-5 shadow-2xl shadow-black/30 sm:p-8">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-2 text-[9px]"><span className="font-black uppercase tracking-wider text-zinc-600">Diferenciação:</span><span className="rounded-md bg-emerald-500/10 px-2 py-1 font-black text-emerald-400">Alta · nenhum elemento igual</span><span className="rounded-md bg-amber-500/10 px-2 py-1 font-black text-amber-400">Média · reutiliza 1 elemento</span><span className="rounded-md bg-red-500/10 px-2 py-1 font-black text-red-400">Baixa · muito semelhante</span></div>
...
function VariationCard({ number, hookSlot, bodySlot, ctaSlot, variation, selectedVariations, selectionFull, onToggle, onPreview }: { number: number; hookSlot: number; bodySlot: number; ctaSlot: number; variation?: Variation; selectedVariations: Variation[]; selectionFull: boolean; onToggle: () => void; onPreview: () => void }) {
  ...
  const differentiationInfo = differentiation === "high" ? { label: "Alta", title: "Bem diferente das combinações selecionadas", style: "bg-emerald-500/10 text-emerald-400" } : differentiation === "medium" ? { label: "Média", title: "Reutiliza um elemento de outra combinação selecionada", style: "bg-amber-500/10 text-amber-400" } : { label: "Baixa", title: "Muito semelhante a outra combinação selecionada", style: "bg-red-500/10 text-red-400" };
