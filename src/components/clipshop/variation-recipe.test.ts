...
test("creates 150 unique and stable visual recipes", () => {
  const first = Array.from({ length: 150 }, (_, index) => buildVariationRecipe(index + 1));
  const second = Array.from({ length: 150 }, (_, index) => buildVariationRecipe(index + 1));
  expect(second).toEqual(first);
  expect(new Set(first.map(variationRecipeSignature))).toHaveLength(150);
});
...
test("classifies differentiation against selected combinations", () => {
  ...
  const duplicate = { ...variation, id: "duplicate", recipe };
  ...
  expect(getVariationDifferentiationLevel(variation, [twoShared])).toBe("medium");
  expect(getVariationDifferentiationLevel({ ...variation, recipe }, [sameTreatment])).toBe("medium");
  expect(getVariationDifferentiationLevel({ ...variation, recipe }, [duplicate])).toBe("low");
});
