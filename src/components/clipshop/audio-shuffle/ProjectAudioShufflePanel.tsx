...
export default function ProjectAudioShufflePanel({ project, onChange }: Props) {
  ...
  return <section id="audio-shuffle" className="mb-8 rounded-[2rem] border border-amber-500/15 bg-gradient-to-br from-zinc-950 to-black p-5 sm:p-8">
    <div className="flex flex-wrap items-start justify-between gap-5"><div><p className="text-[10px] font-black uppercase tracking-[.3em] text-amber-500">02 · Audio Shuffle</p><h2 className="mt-2 text-2xl font-black text-white">Áudio exclusivo para cada variação</h2><p className="mt-2 max-w-2xl text-xs leading-5 text-zinc-500">Prepare até 27 sequências vinculadas às durações reais de Gancho + Corpo + CTA.</p></div><label className="flex cursor-pointer items-center gap-2 rounded-xl bg-amber-400 px-4 py-3 text-xs font-black text-black"><Upload className="h-4 w-4" />Enviar fonte<input type="file" accept="audio/*,.mp3,.wav,.m4a,.aac" className="hidden" onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); event.target.value = ""; }} /></label></div>
